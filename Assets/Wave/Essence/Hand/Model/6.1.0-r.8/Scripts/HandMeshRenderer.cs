...
namespace Wave.Essence.Hand.Model
{
	...
	public class HandMeshRenderer : MonoBehaviour
	{
		...
		{
			if (!InputDeviceHand.IsTracked(IsLeft, out  updatedTimestamp)) return false;

			palmXR = InputDeviceHand.GetPalm(IsLeft);
			wristXR = InputDeviceHand.GetWrist(IsLeft);
			for (int i = 0; i < fingersXR.Length; i++)
			{
				fingersXR[i] = InputDeviceHand.GetFingerBones(IsLeft, (HandFinger)i);
			}
			return true;
		}
		...
		private bool GetWaveBones()
		{
			...
			if (InputDeviceHand.GetWristLinearVelocity(IsLeft, out Vector3 wristVel))
			{
				long timestamp = Stopwatch.GetTimestamp();
				long frequency = Stopwatch.Frequency;
				long timestampInNanoseconds = (long)((double)timestamp / frequency * 1e+9);
				long diffInNanoseconds = timestampInNanoseconds - updatedTimestamp;
				double diffInSeconds = (double)diffInNanoseconds / 1e+9;
				for (int i = 0; i < s_JointPosition.Length; i++)
				{
					s_JointPosition[i] += wristVel * (float)diffInSeconds;
				}
			}
			return true;
		}
		...
		private bool IsPoseValid()
		{
			...
			if (hasIMManager)
			{
				...
				{
					if (useInputDevice)
						isPoseValid = InputDeviceHand.IsTracked(IsLeft);
					else
