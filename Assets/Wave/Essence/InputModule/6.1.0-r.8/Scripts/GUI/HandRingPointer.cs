...
{
	[System.Obsolete("NOT USE CURRENTLY")]
	[DisallowMultipleComponent]
	[RequireComponent(typeof(MeshRenderer), typeof(MeshFilter))]
	sealed class HandRingPointer : MonoBehaviour
	{
		private const string LOG_TAG = "Wave.Essence.InputModule.HandRingPointer";
		private void DEBUG(string msg) { Log.d(LOG_TAG, this.Hand + ", " + msg, true); }
		...
		// ----------- Z distance of ring -----------
		private const float DEF_POINTER_DISTANCE = 0.5f;
		...
		private const string POINTER_MATERIAL_NAME = "Materials/HandRingPointer01";
		...
		void OnEnable()
		{
			if (!mEnabled)
			{
				...
				HandPointerProvider.Instance.SetHandPointer(this.Hand, gameObject);
		...
		void Update()
		{
			...
			/*// -----------------------------------
			pointerFrameOffset.y = pointerDistance / (Mathf.Sqrt(2));
			pointerFrameOffset.z = pointerDistance / (Mathf.Sqrt(2));
