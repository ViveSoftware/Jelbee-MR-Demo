...
namespace Wave.Essence.InputModule
{
	...
	sealed class GazePointer : MonoBehaviour
	{
		...
		void Update()
		{
			...
			ActivatePointer(m_ShowPointer && (m_GazeInputModule && m_GazeInputModule.enabled));

			ValidateParameters();

			// isTracking means the pointer is controlled by the GazeInputModule.
			pointerFrameOffset = isTracking ? pointerLocalOffset : (Vector3.forward * m_PointerDistance);
		...
		/// <summary>
		/// Set the Gaze tracking offset.
		/// </summary>
		/// <param name="tracking"></param>
		/// <param name="offset">The distance from eye origin to target position.</param>
		public void OnTracking(bool tracking, Vector3 localOffset)
		{
			/*DEBUG("OnTracking() tracking: " + tracking
				+ ", localOffset (" + localOffset.x + ", " + localOffset.y + ", " + localOffset.z + ")");*/
			isTracking = tracking;
			pointerLocalOffset = localOffset;
		}
		#endregion
	}
}
