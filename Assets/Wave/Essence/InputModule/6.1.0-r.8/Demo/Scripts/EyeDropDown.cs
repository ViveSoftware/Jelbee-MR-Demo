...
namespace Wave.Essence.InputModule.Demo
{
	...
	{
		const string LOG_TAG = "Wave.Essence.InputModule.Demo.EyeDropDown";
		void DEBUG(string msg) { Log.d(LOG_TAG, msg, true); }

		public GazeInputModule InputModule = null;
		...
		void Update()
		{
			...
			if (InputModule != null && InputModule.Eye != (EyeManager.EyeType)m_DropDown.value)
			{
				DEBUG("Set eye to " + (EyeManager.EyeType)m_DropDown.value);
				InputModule.Eye = (EyeManager.EyeType)m_DropDown.value;
			}
