						...
						if (InputDevices.GetDeviceAtXRNode(node).TryGetFeatureValue(new InputFeatureUsage<float>(travel1DKey[i]), out axis))
						{
							if (travel1DObject.hasAnimation)
							{
								if (travel1DObject.gameObject != null)
								{
									...
									var trans = travel1DObject.gameObject.transform;
									axis = Mathf.Clamp01(axis);
									trans.localPosition = Vector3.Lerp(travel1DObject.originPosition, travel1DObject.pressPosition, axis);
									trans.localRotation = Quaternion.Lerp(travel1DObject.originRotation, travel1DObject.pressRotation, axis);
								}
							}
						}
					}
