...
namespace Wave.Essence.Interaction.Mode.Editor
{
	...
	public class InteractionModeManagerEditor : UnityEditor.Editor
	{
		...
		// ControllerInputModule property
		...
		// HandManager property
		//SerializedProperty m_PriorTracker;
		SerializedProperty m_InitialStartNaturalHand;
		//SerializedProperty m_InitialStartElectronicHand, m_ElectronicHandModel, m_ShowElectronicHandIfSupported;
		private void OnEnable()
		{
			...
			// HandManager property
			//m_PriorTracker = serializedObject.FindProperty("m_PriorTracker");
			m_InitialStartNaturalHand = serializedObject.FindProperty("m_InitialStartNaturalHand");
			/*m_InitialStartElectronicHand = serializedObject.FindProperty("m_InitialStartElectronicHand");
			m_ElectronicHandModel = serializedObject.FindProperty("m_ElectronicHandModel");
			m_ShowElectronicHandIfSupported = serializedObject.FindProperty("m_ShowElectronicHandIfSupported");*/
		}

		bool gazeOptions = true, eyeManagerOptions = true, controllerOptions = true, handOptions = true, handManagerOptions = true;
		...
		public override void OnInspectorGUI()
		{
			...
			eyeManagerOptions = EditorGUILayout.Foldout(eyeManagerOptions, "Eye Manager Options");

			foldoutStyle.fontSize = 12;
			foldoutStyle.fontStyle = FontStyle.Normal;

			if (eyeManagerOptions)
			{
				EditorGUILayout.HelpBox(
					"Enables or disables the eye tracking.",
					MessageType.Info);
				EditorGUILayout.PropertyField(m_EnableEyeTracking);

				EditorGUILayout.HelpBox(
					"Eye Tracking in local space or world space.",
					MessageType.Info);
				EditorGUILayout.PropertyField(m_LocationSpace);
			}

			// ControllerInputModule options
			EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
			...
			if (handOptions)
			{
				...
				if (handInputModuleOptions)
				{
					...
					EditorGUILayout.HelpBox(
						...
						MessageType.Info);
					EditorGUILayout.PropertyField(m_UseDefaultPinch);
			...
			handManagerOptions = EditorGUILayout.Foldout(handManagerOptions, "Hand Manager Options");

			foldoutStyle.fontSize = 12;
			foldoutStyle.fontStyle = FontStyle.Normal;

			if (handManagerOptions)
			{
				/*EditorGUILayout.HelpBox(
					"Select the prior tracker when both natural and electronic trackers are enabled.",
					MessageType.Info);
				EditorGUILayout.PropertyField(m_PriorTracker);*/

				EditorGUILayout.HelpBox(
					"Starts the natural hand tracking by default. Note: You have to check the menu item\n" +
					"Wave > HandTracking > EnableHandTracking",
					MessageType.Info);
				EditorGUILayout.PropertyField(m_InitialStartNaturalHand);

				/*EditorGUILayout.HelpBox(
					"Starts the electronic hand tracking by default. Note: You have to check the menu item\n" +
					"Wave > HandTracking > EnableHandTracking",
					MessageType.Info);
				EditorGUILayout.PropertyField(m_InitialStartElectronicHand);

				EditorGUILayout.HelpBox(
					"Decides the electronic model type.",
					MessageType.Info);
				EditorGUILayout.PropertyField(m_ElectronicHandModel);
